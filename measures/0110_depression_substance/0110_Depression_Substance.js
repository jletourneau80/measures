function() {
	...
	var denominator = function() {
		...
		var diagnoses = normalize(measure.condition_bh_conditions_involving_bipolar_disorder_diagnosis_active, 
			measure.condition_bh_conditions_involving_unipolar_depression_diagnosis_active);
		...
		var treatments = normalize(
			measure.procedure_bh_psychotherapy_procedure_performed, measure.procedure_bh_psychotherapy_procedure_ordered,
			measure.procedure_bh_counseling_for_depression_procedure_performed, measure.procedure_bh_counseling_for_depression_procedure_ordered,
			measure.procedure_bh_electroconvulsive_therapy_procedure_performed, measure.procedure_bh_electroconvulsive_therapy_procedure_ordered,
			measure.medication_bh_anti_depressant_medication_medication_ordered,
		...
		if (diagnoses_during_encounter_during_measurement_period.length == 0 ||
				...
				for (var i = 0; i < diagnoses_during_encounter_during_measurement_period.length; i++) {
					...
					if (diagnoses_during_blackout_period.length == 0 && 
						meds_active_during_encounters_during_blackout_period.length == 0 && 
						treatments_during_blackout_period.length == 0 && 
						treatments_during_followup_period.length > 0) {
